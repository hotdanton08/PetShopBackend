  ...
  up: async (queryInterface, Sequelize) => {
    ...
    for (const orderItem of orderItems) {
      ...
      if (!productMap[orderItem.productName]) {
        const productId = await queryInterface.rawSelect('Products', {
    ...
    for (const email in userMap) {
      ...
      const orderId = await queryInterface.rawSelect('Orders', {
        where: { userId: userId },
      }, ['id']);
    ...
    for (const orderItem of orderItems) {
      ...
      // 檢查資料庫中是否已經存在這個訂單項目
      const existingOrderItem = await queryInterface.rawSelect('OrderItems', {
        where: { orderId: orderItemData.orderId, productId: orderItemData.productId },
      }, ['id']);
      ...
      if (!existingOrderItem) {
        ...
        // 如果訂單項目已經存在，更新數據
        await queryInterface.bulkUpdate('OrderItems', orderItemData, { id: existingOrderItem });
      }
    }
  },
